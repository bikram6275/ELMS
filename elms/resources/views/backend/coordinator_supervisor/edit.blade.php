<div class="card card-default">
    ...
    <div class="card-body">
        ...
        {!! Form::open(['method' => 'PUT','route'=>['coordinator_supervisor.update',$edits->id], 'enctype' => 'multipart/form-data']) !!}

        <div class="form-group {{ $errors->has('coordinator_id') ? 'has-error' : '' }}">
            <label>Coordinator</label>
            {{ Form::select('coordiantor_id', $coordinators->pluck('name', 'id'), $edits->coordinator_id, [
        ...
        <div class="form-group {{ $errors->has('supervisor_id') ? 'has-error' : '' }}">
            ...
            {{ Form::select('supervisor_id', $supervisors->pluck('name', 'id'), $edits->supervisor_id, [
                'class' => 'form-control select2',
                'id' => 'supervisor_id',
        ...
        <div class="form-group {{ $errors->has('survey_id') ? 'has-error' : '' }}">
            <label>Survey</label>
            {{ Form::select('survey_id', $surveys->pluck('survey_name', 'id'),$edits->survey_id, [
                'class' => 'form-control select2',
                'id' => 'survey_id',
