...
<table class="table table-bordered" id=skilledTable>
    ...
    <tbody>
      ...
      @endphp
        ...
        @php
            $occu = $occupations[$value->sector_id];
            $occupi = App\Models\Occupation\Occupation::where('sector_id',null)->first();
            $occu [] = $occupi ;
        @endphp
                ...
                <tr>
                    ...
                    <td>{{Form::select("skilled[$key][occupation_id]",$occu[$value->sector_id]->pluck('occupation_name','id'),$value->occupation_id!=null?$value->occupation_id:null,['class'=>'form-control select2','id'=>"occupation_$key",'placeholder'=>
                        ' Select Occupation','onChange' => 'otherOccupation(this)'])}}

                        {!! $errors->first("skilled.$key.occupation_id", '<span class="text-danger">:message</span>') !!}
                        @if ($value->other_occupation_value != null)
                            {!! Form::text("skilled[$key][other_occupation_value]", $value->other_occupation_value, ['class' => "form-control mt-2 other_$key", 'placeholder' => 'Enter Other Occupation']) !!}
                        @endif
                    </td>
                    ...
                    <td>{!! Form::select("skilled[$key][incorporate_possible]", ['yes'=>'Yes','no'=>'No'], $value->incorporate_possible, ['class' => 'form-control add-required select2']) !!}
    ...
    @else
        <tr>
            ...
            <td>
            <select name="skilled[0][occupation_id]" id="occupation_0" class="form-control select2" onChange=  'otherOccupation(this)'>
            ...
            <td>{!! Form::select('skilled[0][incorporate_possible]', ['yes'=>'Yes','no'=>'No'], null, ['class' => 'form-control add-required select2']) !!}
...
@push('custom-scripts')
    ...
    <script>
        ...
        $('#add1').click(function() {
            ...
            <td>{!! Form::select('skilled[${row_count}][incorporate_possible]',['yes'=>'Yes','no'=>'No'], null, ['class' => 'form-control add-required select2']) !!}</td>
...
<script>
        function otherOccupation(el) {
            ...
            if ($(el).find('option:selected').text() === 'Others') {
                ...
                $(el).parents('td').append(`{!! Form::text('skilled[${count}][other_occupation_value]', null, ['class' => 'form-control mt-2 ${ik}_other', 'placeholder' => 'Enter Other Occupation']) !!}`);
            } else {
                var ik = el.id
                var count = ik.split('_')[1];
