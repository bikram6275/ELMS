...
class CreateEmployeePunishmentsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('employee_punishments', function (Blueprint $table) {
            ...
            $table->string('defence_letter_received')->nullable();
            $table->enum('de_promoted',['1','0'])->nullable()->default('0');
            $table->enum('grade_deducted',['1','0'])->nullable()->default('0');
            $table->integer('year')->unsigned();
            $table->foreign('year')->references('id')->on('fiscal_years');
            $table->string('punishment_img')->nullable();
