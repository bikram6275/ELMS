...
class OrganizationController extends GetxController {
  ...
  RxBool getDistrictsLoading = false.obs;
  RxBool getDistrictsError = false.obs;

  RxInt clickedIndex = 0.obs;

  RxString pradeshId = "".obs;
  RxString districtId = "".obs;
  RxString sectorId = "".obs;

  RxList pradeshList = <PradeshModel>[].obs;
  RxList districtList = <DistrictModel>[].obs;
  RxList sectorList = <SectorModel>[].obs;
  ...
  getPradeshTitle() {
    if (pradeshId.value == "") {
      return "select pradesh";
    } else {
      for (PradeshModel element in pradeshList) {
        if (element.id.toString() == pradeshId.value) {
          return element.pradeshName;
        }
      }
    }
  }
  ...
  getDistrictTitle() {
    if (districtId.value == "") {
      ...
      for (DistrictModel element in districtList) {
        if (element.id.toString() == districtId.value) {
          return element.englishName + " | " + element.nepaliName;
        }
      }
      return "select district";
    }
  ...
  getSectorTitle() {
    if (sectorId.value == "") {
      ...
      for (SectorModel element in sectorList) {
        if (element.id.toString() == sectorId.value) {
          return element.sectorName;
        }
      }
    }
  }

  getFilterPradesh() async {
    try {
      ...
      if (response != null) {
        ...
        for (var element in list) {
          pradeshList.add(PradeshModel.fromJson(element));
        }
      } else {
  ...
  getFilterSector() async {
    try {
      ...
      if (response != null) {
        ...
        for (var element in list) {
          sectorList.add(SectorModel.fromJson(element));
        }
      } else {
        getOrganizationError(true);
      }
  ...
  getFilterDistrict(String? pradeshId) async {
    try {
      getDistrictsError(false);
      getDistrictsLoading(true);
      districtList.clear();

      D.Response? response = await _apiService.get(
        path: _path.districtFilter.replaceAll("#", pradeshId ?? ""),
        needToken: true,
      );
        ...
        if (questionType == "input" && listOfAnswer.isNotEmpty) {
          ...
          for (var element in listOfAnswer) {
            technicalList.add({
              ...
              "other_occupation_value": element["other_occupation_value"],
          ...
          for (var element in listOfAnswer) {
            if (element["skill"] == "communication") {
              ...
              communicationAnswerUT = element["formally_untrained"].toString();
              ...
              interpersonalAnswerUT = element["formally_untrained"].toString();
            }
          }
          ...
          if (conditionalAnswer.isNotEmpty) {
            otherAnswer = conditionalAnswer.last["answer"];
          }
            ...
            question8AnswerList.add(
              {
                ...
                "other_value": element["other_value"]
              },
            );
          }
          ...
          var formData = {
            ...
            "technology": {
              "id": tempSubAnswer["id"],
              "sector_id": tempSubAnswer["sector"]["id"],
